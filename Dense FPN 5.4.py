...
class FeaturePyramidNetwork(nn.Module):
    ...
    def __init__(
            ...
            agent_dim: int = 32,
            agent_num: int = 49  # [新增] 默认 7x7=49 个 Agent
    ):
        super().__init__()
        ...
        for in_ch in in_channels_list:
            ...
            if norm_layer is None:
                ...
                layer = nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1, bias=True)
            else:
                inner = nn.Sequential(
    ...
    def forward(self, x: Dict[str, Tensor]) -> Dict[str, Tensor]:
        if not isinstance(x, (dict, OrderedDict)):
            raise TypeError("FeaturePyramidNetwork expects an OrderedDict as input")
